...
class JsonRpcMessage(object):
    ...
    :param dict json_data: The JSON-RPC message Python representation.
    """
    ...
    def from_json(self, json_data):
        ...
        try:
            ...
            if not set(json_data.keys()) <= JSON_RPC_KEYS:
                raise InvalidRequestError('Invalid members in request object')

            if not ('jsonrpc' in json_data and json_data['jsonrpc'] == '2.0'):
            ...
            if not isinstance(json_data['method'], basestring):
                raise InvalidRequestError('Method must be a string')

            self.method_name = json_data['method']
